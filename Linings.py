...
def get_zprad(c, rho0, j, k, l, n, r, s, length, depth, height_d, M, freq):
                    ...
                    for N in n:
                        ...
                        for R in r:
                            ...
                            for S in s:
                                ...
                                # calculate kappars
                                kappars = np.sqrt(((R*np.pi)/(depth))**2+((S*np.pi)/(height_d))**2)
                                
                                # 
                                Krsp = (-k0*M-1j*np.sqrt((1-M**2)*kappars**2-k0**2))/(1-M**2)
                                Krsm = (k0*M-1j*np.sqrt((1-M**2)*kappars**2-k0**2))/(1-M**2)
                                # j=l
                                if J==L:
...
class SinglePlateResonator3D(PlateResonators):
    ...
    def zmatrix(self, height_d, M, freq):
        ...
        # accelerated by numba
        Zprad = get_zprad(self.cavity.medium.c, self.cavity.medium.rho0, self.j, self.k, self.l, self.n, self.cavity.r, self.cavity.s, self.length, self.depth, height_d, M, freq)
...
#%% 
# class SinglePlateResonator3D_2(PlateResonators):
    
#     '''
#     3D single plate silencer.
#     Calculation of impedance matrix of plate induced sound with loops
#     '''
...
#         # impedance matrix of plate induced sound
#         Zprad = np.zeros((len(self.j), len(self.k), len(self.l), len(self.n), len(freq)), dtype=complex)
...
# class SinglePlateResonator3D_3(PlateResonators):
    
#     '''
#     3D single plate resonator lining.
#     '''
#     # plate
#     plate = tr.Instance(Plate3D)
...
#         # impedance matrix of the plate induced sound
#         # define expanded arrays of modes
#         J = self.j[:, np.newaxis, np.newaxis, np.newaxis, np.newaxis, np.newaxis, np.newaxis].astype(complex)
...
#         # calculate the overall impedance matrix of the plate induced sound
#         Zprad = Zpjkln+Zplkln+Zplrln+Zplklr+Zpjrln+Zpjklr
